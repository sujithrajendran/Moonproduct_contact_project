...
def add():
    if request.method=='GET':
        ...
        if len(str(number))==10 and len(str(number))==10:
            conn=contact(name=name.capitalize(),mobile_no=number,s_no=snumber,email_id=emailid,gender=gender)
            db.session.add(conn)
            db.session.commit()
...
def update():
    if request.method=='GET':
        ...
        if len(str(number))==10 and len(str(number))==10:
            ...
            db.session.add(conn)
            db.session.commit()
            return render_template('updatealert.html')
        else:
            return render_template("alert.html")
...
def search():
    if request.method=='GET':
        ...
        conn=contact.query.filter(contact.name.like(name.capitalize()+'%')).all()
